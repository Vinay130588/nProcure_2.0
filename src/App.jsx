...
import { appPath, assetPath, routePath } from './paths'
import { EMAIL_PATTERN, MOBILE_PATTERN, fieldKind, fieldPattern, normalizeFieldValue, validateField } from './validation'
...
function App() {
  const [route, setRoute] = useState(() => routeFromPath(routePath()))
  ...
  const navigate = (next, data) => {
    ...
    window.history.pushState(data || null, '', appPath(path))
    setRoute(routeFromPath(path.split('?')[0]))
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  useEffect(() => {
    const onPop = () => setRoute(routeFromPath(routePath()))
...
function TenderSearch({ query, setQuery }) {
  const [results, setResults] = useState([]), [loading, setLoading] = useState(true), [category, setCategory] = useState(''), [status, setStatus] = useState(''), [alertEmail, setAlertEmail] = useState(''), [alertEmailTouched, setAlertEmailTouched] = useState(false)
...
function CommonRegistration({navigate}) {
  ...
  const continueToOtp=(e)=>{e.preventDefault();setError('');if(!form.fullName.trim()||!EMAIL_PATTERN.test(form.email)||!MOBILE_PATTERN.test(form.mobile)||form.userId.length<4||form.password.length<8)return setError('Complete all fields with a valid email, 10-digit mobile, User ID and minimum 8-character password.');setStep(1)}
...
function Field({label,type='text',value,onChange,required,error}) {
  ...
  const result = validateField({ label, type, value: currentValue, required })
  const shownError = touched ? (result.error || error) : ''
  const showSuccess = touched && Boolean(String(currentValue).trim()) && result.valid && !error
  const errorId = `${label.replace(/\W+/g, '-').toLowerCase()}-error`
  const change = (event) => {
    const next = normalizeFieldValue(label, type, event.target.value)
    if (onChange) onChange(next)
    else setLocalValue(next)
  }
  const pattern = fieldPattern(kind)?.source
  return <label className={`field validated-field ${shownError?'is-invalid':showSuccess?'is-valid':''}`}><span>{label}{required&&<b> *</b>}</span><span className="validation-control"><input type={kind === 'email' ? 'email' : type} value={currentValue} onChange={change} onBlur={()=>setTouched(true)} required={required} maxLength={{mobile:10,pin:6,pan:10,gst:15}[kind]} inputMode={['mobile','pin'].includes(kind) ? 'numeric' : undefined} pattern={pattern} aria-invalid={!!shownError} aria-describedby={shownError ? errorId : undefined}/>{(shownError||showSuccess)&&<i className="validation-icon" aria-hidden="true">{showSuccess?'✓':'×'}</i>}</span>{shownError&&<small id={errorId} className="error-text" role="alert">{shownError}</small>}</label>
}
