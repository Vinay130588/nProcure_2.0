...
function GovernmentClientRegistration({ navigate, openOther }) {
  ...
  const [requestorVerified, setRequestorVerified] = useState(false);
  const [error, setError] = useState("");
  const [submitted, setSubmitted] = useState(null);
  const update = (setter, key, value) => setter((old) => ({ ...old, [key]: value }));
  const updateRequestorContact = (key, value) => { update(setRequestor, key, value); setRequestorVerified(false); };
  ...
  const submit = (event) => {
    ...
    if (!requestorVerified) return setError("Verify the Authorized Requestor email and mobile OTP before submitting.");
    ...
    setSubmitted(governmentRequestService.submit({ requestor: { ...requestor, emailVerified: true, mobileVerified: true, verifiedAt: new Date().toISOString() }, department, offices, designations, proposedUsers, declarationAccepted: true }));
  ...
  return <main className="flow-page government-request-page">
    ...
    <form onSubmit={submit}>
      <section className="flow-card compact-section"><div className="section-title-row"><div><span>01</span><h2>Authorized Requestor</h2></div>{requestorVerified&&<span className="wf-status success">Email & Mobile Verified</span>}</div><div className="wf-grid"><WFInput label="Authorized Person Name" value={requestor.name} onChange={(v) => update(setRequestor, "name", v)} required/><WFInput label="Designation" value={requestor.designation} onChange={(v) => update(setRequestor, "designation", v)} required/><WFInput label="Official Email" type="email" value={requestor.email} onChange={(v) => updateRequestorContact("email", v)} required/><WFInput label="Mobile Number" value={requestor.mobile} onChange={(v) => updateRequestorContact("mobile", v.replace(/\D/g, "").slice(0, 10))} required/></div><DualOtpVerification key={`${requestor.email}|${requestor.mobile}`} email={requestor.email} mobile={requestor.mobile} verified={requestorVerified} onVerified={()=>{setRequestorVerified(true);setError("");}}/></section>
...
function GoGQueryReplyLegacy({ back }) {
...
function DualOtpVerification({email,mobile,verified,onVerified,title="Verify Registered Contact"}) {
  const [sent,setSent]=useState(false), [emailOtp,setEmailOtp]=useState(""), [mobileOtp,setMobileOtp]=useState(""), [message,setMessage]=useState("");
  const validEmail=/^\S+@\S+\.\S+$/.test(email||""), validMobile=/^\d{10}$/.test(mobile||"");
  const send=()=>{if(!validEmail||!validMobile)return setMessage("Enter a valid email and 10-digit mobile number first.");setSent(true);setMessage(`OTP sent to ${maskEmail(email)} and ${maskMobile(mobile)}.`);};
  const verify=()=>{if(emailOtp!==OTP||mobileOtp!==OTP)return setMessage("Invalid OTP. Enter the correct Email OTP and Mobile OTP.");setMessage("Email and mobile number verified successfully.");onVerified();};
  return <div className={`gog-otp-verification ${verified?"verified":""}`}><div className="gog-otp-head"><div><ShieldCheck/><span><strong>{title}</strong><small>Both Email OTP and Mobile OTP are mandatory.</small></span></div>{verified?<span className="gog-verified-badge"><CheckCircle2/> Verified</span>:<button type="button" className="wf-secondary" disabled={!validEmail||!validMobile} onClick={send}>{sent?"Resend OTP":"Send OTP"}</button>}</div>{sent&&!verified&&<div className="gog-dual-otp-grid"><WFInput label={`Email OTP (${maskEmail(email)})`} value={emailOtp} onChange={(v)=>setEmailOtp(v.replace(/\D/g,"").slice(0,6))} required/><WFInput label={`Mobile OTP (${maskMobile(mobile)})`} value={mobileOtp} onChange={(v)=>setMobileOtp(v.replace(/\D/g,"").slice(0,6))} required/><button type="button" className="wf-primary" disabled={emailOtp.length!==6||mobileOtp.length!==6} onClick={verify}>Verify Both OTPs</button></div>}{message&&<p className={verified?"gog-otp-success":"gog-otp-message"}>{message}</p>}{sent&&!verified&&<small className="demo-otp-note">Prototype OTP for Email and Mobile: <strong>{OTP}</strong></small>}</div>;
}
const maskEmail=(email="")=>{const [name,domain]=email.split("@");if(!domain)return email;return `${name.slice(0,2)}${"*".repeat(Math.max(2,name.length-2))}@${domain}`;};

function GoGQueryReply({back}) {
  const [requestNumber,setRequestNumber]=useState(""), [pending,setPending]=useState(null), [query,setQuery]=useState(null), [verified,setVerified]=useState(false), [reply,setReply]=useState(""), [supporting,setSupporting]=useState([]), [additional,setAdditional]=useState([]), [error,setError]=useState(""), [success,setSuccess]=useState("");
  const searchRequest=(event)=>{event.preventDefault();setError("");setSuccess("");setPending(null);setQuery(null);setVerified(false);const found=governmentRequestService.find(requestNumber);const open=found?.queryHistory?.find((item)=>item.status==="OPEN");if(!found)return setError("Valid GoG Request Number was not found.");if(!open)return setError("This request has no pending clarification query.");setPending(found);setQuery(open);};
  const files=(fileList)=>Array.from(fileList||[]).filter((file)=>file.size<=5*1024*1024&&["application/pdf","image/jpeg","image/png"].includes(file.type)).map(fileMeta);
  const submit=()=>{setError("");if(!verified)return setError("Verify the registered Email and Mobile OTP before replying.");const result=governmentRequestService.replyToQuery(pending.requestNumber,{reply,supportingDocuments:supporting,additionalDocuments:additional});if(result.error)return setError(result.error);setSuccess(`Reply submitted for ${query.queryNumber}.`);setPending(null);setQuery(null);setReply("");setSupporting([]);setAdditional([]);setVerified(false);};
  const email=pending?.applicantEmail||pending?.submittedData?.requestor?.email||"", mobile=pending?.submittedData?.requestor?.mobile||"";
  return <main className="flow-page gog-query-reply"><button className="wf-secondary" onClick={back}><ChevronLeft/> Back to GoG Request</button><div className="flow-heading"><span>GoG Client Account Request</span><h1>Query Reply</h1><p>Search the Request Number, verify its registered Email and Mobile OTP, then submit the clarification reply.</p></div><form className="flow-card gog-query-search" onSubmit={searchRequest}><WFInput label="GoG Request Number" value={requestNumber} onChange={setRequestNumber} required/><button className="wf-primary"><Search/> Search Request</button></form>{error&&<p className="wf-error">{error}</p>}{success&&<section className="flow-success"><CheckCircle2/><h2>Clarification reply submitted</h2><p>{success}</p></section>}{pending&&query&&!verified&&<section className="flow-card gog-query-auth-card"><div className="gog-admin-summary"><div><small>Request Number</small><strong>{pending.requestNumber}</strong></div><div><small>Department</small><strong>{pending.organizationName}</strong></div><div><small>Registered Email</small><strong>{maskEmail(email)}</strong></div><div><small>Registered Mobile</small><strong>{maskMobile(mobile)}</strong></div></div><DualOtpVerification key={pending.id} email={email} mobile={mobile} verified={verified} title="Authenticate Query Reply Access" onVerified={()=>{setVerified(true);setError("");}}/></section>}{pending&&query&&verified&&<section className="flow-card gog-query-reply-card"><div className="gog-admin-summary"><div><small>Request Number</small><strong>{pending.requestNumber}</strong></div><div><small>Organization</small><strong>{pending.organizationName}</strong></div><div><small>Authentication</small><strong>Email & Mobile OTP Verified</strong></div><div><small>Query Reference</small><strong>{query.queryNumber}</strong></div></div><GoGSection title="Pending Query"><p><strong>{query.subject}</strong> · {query.category}</p><p>{query.details}</p><small>{new Date(query.createdAt).toLocaleString()}</small></GoGSection><label className="wf-field"><span>Reply Details *</span><textarea value={reply} onChange={(event)=>setReply(event.target.value)}/></label><div className="wf-grid"><label className="upload-v2">Supporting Documents<input type="file" multiple accept=".pdf,.jpg,.jpeg,.png" onChange={(event)=>setSupporting(files(event.target.files))}/><span>{supporting.length} file(s)</span></label><label className="upload-v2">Additional Documents<input type="file" multiple accept=".pdf,.jpg,.jpeg,.png" onChange={(event)=>setAdditional(files(event.target.files))}/><span>{additional.length} file(s)</span></label></div><p className="wf-note">PDF/JPG/PNG only, maximum 5 MB per file.</p><button className="wf-primary" disabled={!reply.trim()} onClick={submit}>Submit Reply</button></section>}</main>;
}
...
const isMeaningfulGoGIntegration = (item) => Boolean(item?.systemName?.trim() && item?.purpose?.trim()) && !(item.systemName === "Configured Integration" && item.purpose === "Account Integration");
...
function GoGStructureTabPro({config,setConfig,readOnly}) {
  ...
  const save=(record)=>{const normalized=(value="")=>value.trim().replace(/\s+/g," ").toLowerCase(), duplicate=units.some((unit)=>unit.id!==record.id&&unit.unitType===record.unitType&&(record.unitType==="Division"?normalized(unit.division)===normalized(record.division):record.unitType==="Sub-Division"?normalized(unit.subDivision)===normalized(record.subDivision):unit.unitType==="Department"));if(duplicate){window.alert(`${record.unitType} '${record.unitType==="Division"?record.division:record.subDivision}' is already configured in this application.`);return;}setConfig((old)=>({...old,departmentStructures:old.departmentStructures.some((x)=>x.id===record.id)?old.departmentStructures.map((x)=>x.id===record.id?record:x):[...old.departmentStructures,record]}));setEdit(null);};
